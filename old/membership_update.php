		...
		try{
			...
			if(empty($MemberName_err) && empty($CompanyName_err) && empty($Phone_err) && empty($Email_err) && empty($Address_err) && empty($Business_err) && empty($Message_err) && empty($Image_load_err))
			{
                if($_FILES["upImage"]["size"] == 0) 
                {
                    ...
                    // Prepare an update statement without save image
                    $sql = "update memberdetails set MemberName = :MemberName, CompanyName = :CompanyName, Phone = :Phone, Email = :Email, Address = :Address, Business = :Business, Message = :Message where MemberID = :MemberID";
                }
                else
                {
				    // Prepare an update statement
				    $sql = "update memberdetails set MemberName = :MemberName, CompanyName = :CompanyName, Phone = :Phone, Email = :Email, Address = :Address, Business = :Business, Message = :Message, imageData = '$file' where MemberID = :MemberID";
                }
				...
				if($stmt = $pdo->prepare($sql))
				{
					...
					$stmt->bindParam(":MemberID", $param_MemberID);
...
<head>
    ...
    <title>DCCI :: Member Update</title>
    <!-- Web Fonts -->
							...
							<div class="col-md-12">
								...
								<form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        ...
                                        <div class="col-md-4">
									        <div class="form-group <?php echo (!empty($Image_load_err)) ? 'has-error' :''; ?>">
                                                <?php echo '<img src="data:image/jpeg;base64,'.base64_encode($row['imageData']).'" class="imagesize" id="blah"/>' ?><br/>
										        <!--<span class="help-block"><?php echo $Image_load_err ?></span>-->
									        </div>
                                        </div>
                                        <div class="col-md-4">
									...
									<input type="submit" class="btn btn-primary" value="Submit" id="insert"/>
                                                                        <a href="membership_details_Copy.php" class="btn btn-info">Cancel</a>
								</form>
							 </div>
						</div>        
					</div>
